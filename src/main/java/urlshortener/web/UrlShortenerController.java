...
public class UrlShortenerController {
	 ...
	 * <p>
	 * This method checks using Google Safe Browsing if original url is save. If it
	 * is, redirects the user. If it is not, redirects to a page that informs the
	 * using about the danger. If an error occurs, it returns a page showing the
	 * specific error.
	 * 
	 ...
	 * <p>
	 * Checks if the provided url is reachable, if it is, returns the shortened url
	 * to the user. If it is not, informs the user about it so he/she can confirm
	 * that wants to shorten the url. It also checks if url is safe using Google
	 * Safe Browsing. If an error occurs, it returns a page showing the specific
	 * error.
	 * 
	...
	public ResponseEntity<ShortURL> shortener(@RequestParam(value = "url", required = true) String url,
		...
		if (urlValidator.isValid(url)) {
			...
			RestTemplate rest = restTemplateBuilder.setConnectTimeout(Duration.ofMillis(1500))
					.setReadTimeout(Duration.ofMillis(1500)).build();
	...
	@ResponseBody
	public ResponseEntity<ShortURL> shortenerConfirm(@RequestParam("url") String url, HttpServletRequest request) {
