...
public class ClickRepositoryImpl implements ClickRepository {
	...
	public CompletableFuture<List<Country>> retrieveCountries(String hash) {
		...
		try {
			return CompletableFuture.completedFuture(jdbc.query(
	...
	public CompletableFuture<List<Country>> retrieveCountriesGlobal() {
		...
		try {
			return CompletableFuture.completedFuture(jdbc.query(
					"SELECT COUNT(id), country, gc FROM click GROUP BY country,gc ORDER BY COUNT(id) DESC", coMapper));
		} catch (Exception e) {
	...
	public CompletableFuture<List<Browser>> retrieveBrowsersGlobal() {
		...
		try {
			return CompletableFuture.completedFuture(jdbc
					.query("SELECT COUNT(id), browser FROM click GROUP BY browser ORDER BY COUNT(id) DESC", boMapper));
		} catch (Exception e) {
			log.debug("When select for key {}", e);
			return null;
		}
	...
	public CompletableFuture<List<Platform>> retrievePlatformsGlobal() {
		...
		try {
			return CompletableFuture.completedFuture(jdbc.query(
					"SELECT COUNT(id), platform FROM click GROUP BY platform ORDER BY COUNT(id) DESC", ptMapper));
		} catch (Exception e) {
			log.debug("When select for key {}", e);
			return null;
		}
	...
	public CompletableFuture<Long> count() {
		try {
			return CompletableFuture.completedFuture(jdbc.queryForObject("select count(*) from click", Long.class));
		} catch (Exception e) {
